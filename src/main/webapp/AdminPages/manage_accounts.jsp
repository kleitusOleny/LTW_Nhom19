                            ...
                            <c:forEach var="user" items="${listAccount}">
                                <tr class="accounts">
                                    ...
                                    <td class="cell-action">
                                        <button class="edit btn edit-btn-trigger"
                                            data-target="modal-edit-${user.id}">Sửa</button>
                                        <c:choose>
                                            <c:when test="${user.active == 1}">
                                                <%-- Đang hoạt động --%>
                                                    <button class="lock btn"
                                                        onclick="toggleUserStatus(${user.id}, 'block')">Khoá</button>
                                            </c:when>
                                            <c:otherwise>
                                                <%-- Đang bị khoá --%>
                                                    <button class="unlock btn"
                                                        onclick="toggleUserStatus(${user.id}, 'unlock')">Mở</button>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
                                        ...
                                        <form action="${pageContext.request.contextPath}/accountmanager/edit"
                                            ...
                                            <div class="edit-information-account">
                                                ...
                                                <div class="username-section">
                                                    ...
                                                    <input type="text" id="username_" name="username_"
                                                        value="${user.username}" placeholder="Chưa có tên đăng nhập">
                                                </div>
                                                <div class="phone-section">
                                                    <label for="phone-number">Số Điện Thoại</label>
                                                    <input type="tel" id="phone-number" name="phone-number"
                                                        value="${user.phoneNumber}" placeholder="Chưa có số điện thoại"
                                                        required>
                                                </div>
                                                <div class="active-section">
                                                    <label for="activeSelect">Đang hoạt động:</label>
                                                    <select id="activeSelect" name="activeSelect" required>
                                                        <option value="1" ${user.active==1 ? 'selected' : '' }>Có
                                                        </option>
                                                        <option value="0" ${user.active==0 ? 'selected' : '' }>Không
                                                        </option>
                                                    </select>
                                                </div>
                                                <div class="administrator-section">
                                                    <label for="administratorSelect">Là Quản Trị Viên:</label>
                                                    <select id="administratorSelect" name="administratorSelect"
                                                        required>
                                                        <option value="1" ${user.administrator==1 ? 'selected' : '' }>Có
                                                        </option>
                                                        <option value="0" ${user.administrator==0 ? 'selected' : '' }>
                                                            Không</option>
                                                    </select>
                                                </div>
                                                <div class="create_account-section">
                                                    ...
                                                    <input type="date"
                                                        value="<fmt:formatDate value='${user.createdAt}' pattern="
                                                        yyyy-MM-dd" />" disabled>
                                                </div>
                                            </div>
                                                ...
                                                <c:if test="${editingId == user.id and not empty errorList}">
                                                    <ul class="error-list-summary">
                                                        <c:forEach var="msg" items="${errorList}">
                                                            <li><ion-icon name="alert-circle-outline"></ion-icon> ${msg}
                                                            </li>
                                                        </c:forEach>
                                                    </ul>
                                                </c:if>
                                            ...
                                            <div class="group-button-action section">
                                                <button type="button" class="cancel element-button close-edit-modal"
                                                    id="close-modal-btn7">Huỷ</button>
            ...
            <form id="add-form" action="${pageContext.request.contextPath}/accountmanager/add" method="POST">
                <div class="username-input">
                    <label for="username" class="label-with-icon">
                        ...
                        Tài Khoản</label>
                    <input type="text" id="username" name="email" placeholder="Nhập Email" value="${param.email}"
